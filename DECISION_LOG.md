...
- Evidence/test: `screen.getByRole('region', { name: 'Scope Lock' })` assertions across `panel-switcher.test.tsx`, `manual-edit.test.tsx`, `reset-project.test.tsx`, `e2e/smoke.spec.ts`, and `e2e/reviewer-journey.spec.ts`.
- Related step/commit: UI revision after Step 12.

### 2026-08-28 - The canvas is permanent; Design, Code, and Layers share one dock

- Context: the previous revision put Preview and Code in a tablist, so the rendered template was unmounted whenever the code view was open, and Design and Layers were two independent toggles that could both claim the right edge at once. Reviewing a change meant switching back to see it, and the canvas width depended on how many docks happened to be open.
- Options considered: keep the tablist and move only the docks to a single-select control (rejected - the preview would still vanish behind the code view, which is the part that hurt); allow a "no panel" state so the dock can be dismissed (rejected - it reintroduces the multi-state the switcher exists to remove, and the canvas is already the resting surface); one permanent canvas plus one dock chosen by a segmented switcher (chosen).
- Decision: the centre renders the template and nothing else. Design, Code, and Layers are a single segmented control in the toolbar - the same shape as the viewport switcher, exactly one item pressed, Design by default - and each fills the one right-hand dock. All three stay mounted and hidden, and the dock has no close control, because there is no state in which no panel is docked.
- Why: what is under review is never hidden by a tool used to change it, the canvas has one predictable width, and three mutually exclusive choices are stated as one control rather than as toggles that can contradict each other. Nothing about the commit boundary moved: the panels own no document state, and canvas, code, AI, and restore still share the store's one validated pipeline.
- Trade-off: the Code panel now reads in a 440 px dock rather than a 1040 px centre column, so long JSON wraps more; Escape no longer dismisses a panel, so the dock always costs its width above 1100 px; and reading a tree and editing a property are now two clicks apart rather than side by side.
- Evidence/test: `src/editor/panel-switcher.test.tsx` (one pressed control, one visible dock, no state lost across a switch, the layers tree still owns Escape), `src/editor/canvas-code.test.tsx` (a draft survives a panel switch; the code panel commits while the canvas stays on screen), `e2e/accessibility.spec.ts` (whole journey by keyboard, axe on each docked panel).
- Related step/commit: UI revision after Step 12.
