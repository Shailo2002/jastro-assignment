...
- Recent work is the one exception, and it unmounts rather than reducing to icons: the section is a list of project names, and a stack of identical clocks names nothing a reader could choose between. Dropping it from the tab order costs no route, because the card's own action opens the same project.
- The signed-in identity survives the collapse, and does not move through it. The avatar stands in the same column as the filter icons at both rail widths, and the name and address beside it take their width to zero as they fade - a text box left standing at its open width would push the avatar out through the rail's overflow, and a rail that says who it belongs to at one width must not go anonymous at the other.
