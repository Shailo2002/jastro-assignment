...
- [x] Editor shell is usable at 1280 px wide; switching between the Design, Code, and Layers panels keeps the selection, the code draft, and the pending AI run (`e2e/smoke.spec.ts`, `e2e/accessibility.spec.ts`, `src/editor/panel-switcher.test.tsx`).
...
- [x] Controls have programmatic labels; axe reports no serious or critical findings on the gallery, on each docked panel with the canvas and the rail on screen, or on the reset dialog (`e2e/accessibility.spec.ts`).
...
- [x] Status and error information is not conveyed by color alone (every state is carried by text plus an inverted fill or icon, never by hue alone; `src/styles/tokens.test.ts` also bans emoji icons and raw colour in components).
